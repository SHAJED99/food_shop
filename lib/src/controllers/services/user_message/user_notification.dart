...
class NotificationService {
  ...
  Future<void> initNotification() async {
    AndroidInitializationSettings initializationSettingsAndroid = const AndroidInitializationSettings('food_panddu');
  ...
  Future showNotificationAfterTime({int id = 0, String? title, String? body, String? payLoad, Duration duration = const Duration(seconds: 5)}) async {
    tz.initializeTimeZones();
    final location = tz.getLocation(tz.local.name);
    final scheduledTime = tz.TZDateTime.now(location).add(duration);
